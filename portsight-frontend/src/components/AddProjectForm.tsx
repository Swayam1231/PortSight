...
export default function AddProjectForm({ onProjectAdded }: { onProjectAdded: () => void }) {
  ...
  const [formData, setFormData] = useState({
    ...
    managerId: '' // We will paste the ID here!
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onProjectAdded(); // Refresh the dashboard
    } catch (error) {
      console.error(error);
      alert('Failed to create project. Did you paste a valid Manager ID?');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 mt-8">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <input className="border p-2 rounded bg-blue-50" placeholder="Paste Manager ID Here" required 
